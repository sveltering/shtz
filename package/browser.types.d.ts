...
export type storeResponseValue<V, B> = [B] extends [false] ? Writable<{
    loading: true;
    error: false;
    success: false;
} | {
...
type ReplaceFunctionReturnStore<Fn, B> = Fn extends (...a: infer A) => Promise<infer R> ? (...a: A) => storeResponseValue<R, B> : Fn;
type RecursiveReplaceFunctionReturnsStore<Obj extends object, B extends boolean> = {
    [Key in keyof Obj]: Obj[Key] extends Function ? ReplaceFunctionReturnStore<Obj[Key], B> : Obj[Key] extends {
        [Key2: string]: any;
    } ? RecursiveReplaceFunctionReturnsStore<Obj[Key], B> : Obj[Key];
};
...
export type browserOCC<T extends AnyRouter> = RecursiveReplaceFunctionReturnsOrUndefined<ReturnType<typeof createTRPCProxyClient<T>>>;
...
export type loadCC<T extends AnyRouter> = (event: LoadEvent) => ReturnType<typeof createTRPCProxyClient<T>>;
...
export type storeCC<T extends AnyRouter, B extends boolean> = RecursiveReplaceFunctionReturnsStore<ReturnType<typeof createTRPCProxyClient<T>>, B>;
