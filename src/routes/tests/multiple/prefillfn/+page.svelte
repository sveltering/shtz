<script>
	...
	const multiple = storeClient.tests.addToList.mutate.$multiple({
		...
		methods: {
			poop: function (response, actions) {
				if (response.success) {
					response.data.date = "POOOOOP";
					actions.update();
				}
			},
			remooove: function (response, actions) {
				actions.remove();
			},
		},
	});
...
{#if $multiple.responses.length}
	{#each $multiple.responses as response}
		...
		{:else if response.success}
			...
			date: {item.date}<br />
			item: {item.item}
			<button on:click={response.poop}>Poop</button><br />
			<button on:click={response.remooove}>remooove</button><br /><br />
		{:else}
			Error adding item
		{/if}
		<br />
