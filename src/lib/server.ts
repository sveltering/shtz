...
import type { HTTPResponse, TRPCOpts, TRPCInner, TRPCErrorOpts, CreateContextType, KeyValueObject, StringLiteral } from "./types.js";
...
export const serverClientCreate = function <R extends AnyRouter>(
    ...
    return async function (event: RequestEvent | ServerLoadEvent): Promise<ReturnType<R["createCaller"]>> {
        return t?._routes?.createCaller?.(await t.context(event, false)) as ReturnType<R["createCaller"]>;
    };
};
